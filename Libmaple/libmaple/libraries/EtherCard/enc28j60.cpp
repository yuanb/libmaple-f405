...
static byte Enc28j60Bank = 0;
static int gNextPacketPtr;
static byte selectPin;

void ENC28J60::initSPI () {
	/*
	 * STM32 Port - GPIO initialize occurs within
...
static void xferSPI (byte data) {
    ...
    */
	// STM32 Port
	ENC28J60::spi->transfer(data);
}

static byte readOp (byte op, byte address) {
	byte result;
	enableChip();
    ENC28J60::spi->transfer(op | (address & ADDR_MASK));
    if ( address & 0x80 ) {
    	ENC28J60::spi->transfer(0);
    }
    result = ENC28J60::spi->transfer(0);
    disableChip();
    return result;
}
...
// STM32 Port - new helper function

void ENC28J60::internal_initialize() {

    // Configure clock (when master) at somewhat less than the maximum,
    // which is 20Mhz.  On the arduino platform, this is used at 8Mhz
    // MSB First,
    // CPOL:  Low when idle, CPHA:  sample on leading (first) edge

#if CYCLES_PER_MICROSECOND == 72
    spi->begin(SPI_9MHZ,MSBFIRST,SPI_MODE_0);
#elif CYCLES_PER_MICROSECOND == 168
    spi->begin(SPI_10_5MHZ,MSBFIRST,SPI_MODE_0);
#endif
...
byte ENC28J60::initialize (word size, const byte* macaddr, byte csPin) {
    ...
    internal_initialize(); // STM32 Port

    // STM32 Port - the chip select pin is managed and
    // configured by the HardwareSPI object.

    //selectPin = csPin;
    //pinMode(selectPin, OUTPUT);

    selectPin = spi->nssPin();
...
word ENC28J60::packetReceive() {
    ...
    if (readRegByte(EPKTCNT) > 0) {
        ...
        // STM32 Port - fixup fixed-size header structure

        struct {
            uint16_t nextPacket; // was word
            uint16_t byteCount;  // was word
            uint16_t status;     // was word
        } header;

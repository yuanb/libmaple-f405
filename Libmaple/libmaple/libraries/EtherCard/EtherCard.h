...
//#include <avr/pgmspace.h> REMOVED in STM32 Port
#include "enc28j60.h"
#include "net.h"
...
class Stash : public /*Stream*/ Print, private StashHeader {
  ...
  typedef struct {
    union {
      uint8_t bytes[64];
      processor_word words[16]; // STM32 PORT - was word words[32]
      struct {
        StashHeader head;
        uint8_t filler[59];
        uint8_t tail;
...
public:
  ...
  // STM32 Portability - the second printIP() is redundant

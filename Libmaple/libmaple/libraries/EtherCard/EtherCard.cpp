...
#include "EtherCard.h"
#include <stdarg.h>
//#include <avr/eeprom.h>  STM32 Port - removed eeprom support
...
static char* wtoa (processor_word value, char* ptr) {
  if (value > 9)
...
void Stash::prepare (PGM_P fmt, ...) {
  Stash::load(0, 0);
  processor_word* segs = Stash::bufs[0].words;
  *segs++ = strlen_P(fmt);
  *segs++ = (processor_word) fmt;
  va_list ap;
  va_start(ap, fmt);
  for (;;) {
    ...
    if (c == 0)
      break;
    if (c == '$') {
      processor_word argval = va_arg(ap, processor_word), arglen = 0;
      switch (pgm_read_byte(fmt++)) {
        ...
        // STN32 Port - removed EEPROM support
        /*
        case 'E': {
          byte* s = (byte*) argval;
          char d;
...
void Stash::extract (word offset, word count, void* buf) {
  ...
  Stash stash;
  char mode = '@', tmp[12], *ptr = NULL, *out = (char*) buf;
  for (word i = 0; i < offset + count; ) {
    ...
    switch (mode) {
      ...
      case 'F':
        ...
        // STM32 Port - removed eeprom support
...
void BufferFiller::emit_p(PGM_P fmt, ...) {
    ...
    for (;;) {
        ...
        switch (c) {
            case 'D':
                wtoa(va_arg(ap, processor_word), (char*) ptr);
                break;
            #ifdef FLOATEMIT
            case 'T':
            ...
            #endif
            case 'H': {
                char p1 =  va_arg(ap, processor_word);
                char p2;
                p2 = (p1 >> 4) & 0x0F;
                p1 = p1 & 0x0F;
            ...
            case 'L':
            	...
            	// STM32 Portability - replace ltoa with
            	// portable sprintf

                //ltoa(va_arg(ap, long), (char*) ptr, 10);
        		sprintf((char *)ptr,"%ld",va_arg(ap, long));
